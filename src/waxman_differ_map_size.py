...
from utils import extract_endnode_file_name, graph_plot, extract_endnode_file_name_differ_map_size
...
def construct_waxman_different_map_size(endnodes_graph_file, degree, edge_len):
    node_locs = []
    ...
    # format: endnodesLocs-2000-100-0.json 
    ...
    with open(dirPath + "deepPlace-" + str(map_size) + '-' + str(endnode_num) + '-' + str(topo_idx) + '.json', "r") as f:
    ...
    with open(dirPath + "waxman-" + str(map_size) + '-' + str(endnode_num) + '-' + str(topo_idx) + '.json', 'w') as f:
