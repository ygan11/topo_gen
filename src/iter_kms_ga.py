...
class KmsGa:
    ...
    def get_start_graph(self, G):
        ...
        for node in G.nodes:
            if G.nodes[node]['type'] == 'endnode' and G.degree(node) == 0:
                ...
                candi = []
                for r in G.nodes:
                    if G.nodes[r]['type'] == 'repeater' and abs(G.nodes[r]['pos'][0] - endnode_pos[0]) <= 2 * step_size and abs(G.nodes[r]['pos'][1] - endnode_pos[1]) <= 2 * step_size:
                        candi.append(r)
    ...
    def optimize_repeater_pos_light_t(self, P, endnodes, l_er, l_rr):
        ...
        def evaluate(individual, graph):
            ...
            # 计算地图中心
            all_positions = endnodes_positions + repeaters_positions
            center_x = map_size / 2#sum(pos[0] for pos in all_positions) / len(all_positions)
            center_y = map_size / 2 #sum(pos[1] for pos in all_positions) / len(all_positions)
